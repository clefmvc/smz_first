<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/application_for_repair/table','set') || $check_action('/application_for_repair/view','set') || $check_action('/application_for_repair/view','get') 
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/application_for_repair/table','set') || $check_action('/application_for_repair/view','set') || $check_action('/application_for_repair/view','get')"
						...
						<!--跨表按钮-->
										<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/repair_record/view')" v-if="($check_action('/repair_record/table','add') || $check_action('/repair_record/view','add')) && scope.row['examine_state'] == '已通过' && !scope.row['repair_record_limit']">
						<span>维修记录</span>
					</el-button>
											<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/application_for_repair/table', 'examine')">
						<span>审核</span>
					</el-button>
				</div>
				</template>
